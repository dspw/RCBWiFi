...
void RcbWifi::timerCallback(int timerID) //timerCallback()
{
    ...
    // timer 1 is used with TIMER command
    if (timerID == 1)
    {
        //if (bCastMode == 1)
        // {
    ...
    // timer 2 is used with TIMERPULSE command
    if (timerID == 2)
    {
...
bool RcbWifi::updateBuffer()
{
    ...
    if (magicNum == 0xc5) // is a good packet
    {
        ...
        int auxDiv = num_samp/4;
        int auxRem = num_samp % 4;
        //LOGC("auxDiv = ",auxDiv, " auxRem = ", auxRem);
        ...
        for (int i = 0; i < num_channels; i++)  // num_channels + 3 for aux
        {
            ...
            {
                //     LOGC(" j = ", j, (i + 22), (j * (num_channels + 2)));
                convbuf[k++] = 0.195 * (float)(recvbuf[ (22 + i) + (j * (num_channels + 2))] - 32768);
        ...
        for (int i = 0; i < num_samp; i++)
        {
            ...
            { //LOGC("eventStateEnable = ",sampleEventEnableState);
                    ...
                    // if ((total_samples + i) % (10*(0+1)) == 0) //testcase
                {
                    if ((eventStateTest & smplEventNum) == 0)
                        ...
                        eventStateTest = 0;
                } //LOGC("eventStateTest = ",eventStateTest);
...
/*    int k = 0;
 ...
 //LOGC(" j = ", j);
 convbuf[k++] = 0.195 * (float)(recvbuf[(j + 22) + (i * (num_channels + 2))] - 32768);
 // convbuf[k++] = 0.195 * (float)(recvbuf[((j * num_samp) + 22) + (0 * (num_channels + 2))] - 32768);
 //LOGC("k = ", k);
 }
